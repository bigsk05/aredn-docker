...
case "$ARCH" in
    x86_64)
        ...
        # Mirror the arm64 branch's network-vs-not-published distinction: an HTTP
        # 404 means the release genuinely ships no x86_64 rootfs; HTTP 000
        # (DNS/connect/TLS/timeout) is a probe/network issue — the workflow
        # summary flags that as infra and the next cron retries.
        code="$(curl -sS -I --retry 2 --max-time 30 -o /dev/null -w '%{http_code}' "$URL" 2>/dev/null || echo 000)"
        case "$code" in
            200)
                printf 'ready=yes\nurl=%s\nreason=\n' "$URL"
                ;;
            404)
                ...
                ;;
            *)
                printf 'ready=no\nurl=\nreason=x86_64 probe failed (HTTP %s, network issue): %s\n' "$code" "$URL"
                ;;
        esac
        ;;
    arm64|armv7l)
