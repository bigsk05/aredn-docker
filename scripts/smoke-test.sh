...
#
# Usage: smoke-test.sh <image[:tag]> [node-name] [node-password]
# The node name/password are CI-assigned test credentials for this smoke node
# only — never gateway/operator credentials.
set -eu

IMAGE="${1:?usage: smoke-test.sh <image> [node-name] [node-password]}"
NODE_NAME="${2:-CI-SMOKE-01}"
NODE_PASS="${3:-smoke123}"
...
docker exec "$NAME" /usr/local/bin/firstuse_setup "$NODE_NAME" "$NODE_PASS" >/dev/null
...
docker exec "$NAME" sh -c 'grep -q "smoothing-half-life 0" /var/etc/babel-active.conf' \
    || { echo "SMOKE FAIL: no smoothing-half-life 0 in /var/etc/babel-active.conf" >&2; exit 1; }
